...
# Optuna（ArcFace/距離学習のハイパパラメータ最適化）

ArcFace を用いた距離学習（`src/metric/LabelClustering_arcface.py`）も Optuna で最適化できます。

- 実行スクリプト: `src/hyper_optimizer/ho_labelclustering_arcface.py`
- 学習本体: `src/metric/LabelClustering_arcface.py`
...
MPLBACKEND=Agg /home/tobeson/miniconda3/envs/traf_ana/bin/python src/hyper_optimizer/ho_labelclustering_arcface.py \
	...
	--study-name lc_arcface_small_loc1-6
```

## 2. 探索する項目（デフォルト）

- 共通: `batch_size`, `lr`, `epochs`, `arcface_s`, `arcface_m`
- スペクトログラム時のみ: `n_fft`, `hop_length`, `mel`

## 3. 探索空間を JSON で制御する（おすすめ）

テンプレ: `./configs/optuna_labelclustering_arcface_hpo.json`
...
# Optuna（Log-Ratio/連続値距離学習のハイパパラメータ最適化）

LogRatioLoss を用いた連続値距離学習（`src/metric/ContinuousLearning.py`）も Optuna で最適化できます。

- 実行スクリプト: `src/hyper_optimizer/ho_continuouslearning.py`
- 学習本体: `src/metric/ContinuousLearning.py`

## 1. 基本実行

```bash
MPLBACKEND=Agg /home/tobeson/miniconda3/envs/traf_ana/bin/python src/hyper_optimizer/ho_continuouslearning.py \
	--model small \
	--data-selection loc1-6 \
	--data-csv ./data/processed/datasets/data_1-6.csv \
	--main-data-dir ./data/processed/datasets \
	...
	--study-name cl_small_loc1-6
```

## 2. 探索する項目（デフォルト）

- 共通: `batch_size`, `lr`, `epochs`, `norm_p`, `eps`
- スペクトログラム時のみ: `n_fft`, `hop_length`, `mel`

## 3. 探索空間を JSON で制御する（おすすめ）

テンプレ: `./configs/optuna_continuouslearning_hpo.json`
