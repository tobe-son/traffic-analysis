...
MPLBACKEND=Agg python src/hyper_optimizer/ho_labelclustering.py \
  ...
  --max-epochs 200 \
  --hpo-config ./configs/optuna_labelclustering_hpo.json \
  --storage sqlite:///outputs/hpo_labelclustering_vgg11.db \
...
#  --max-epochs 200 \
#  --hpo-config ./configs/optuna_labelclustering_hpo.json \
#  --storage sqlite:///outputs/hpo_labelclustering_resnet.db \
#  --study-name cl_optuna_resnet_loc1-6_v1 \
#  --export-best-weights  \
#  --reset-study

  # resnet18のoptuna最適化
PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True MPLBACKEND=Agg python src/hyper_optimizer/ho_labelclustering.py \
  --model resnet18 \
  --data-selection loc1-6 \
  --data-csv ./data/processed/datasets/data_1-6.csv \
  --main-data-dir ./data/processed/datasets \
