"""Optuna-based HPO for ArcFace metric learning (LabelClustering_arcface).

This script runs ArcFace metric learning using the training logic in
``src/metric/LabelClustering_arcface.py`` and optimizes the validation loss.
...
from metric.LabelClustering_arcface import (
    DEFAULT_HOP_LENGTH,
    ...
    set_global_seed,
    train_one_epoch_arcface,
)
from metric.utils import extract_embedding


def _train_one_epoch_arcface_amp(
    ...
    if usable_batches == 0:
        raise RuntimeError("No valid batches produced ArcFace loss. Check dataset or batch size.")
...
def parse_hpo_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Optuna-based HPO for ArcFace metric learning")
    ...
    parser.add_argument("--arcface-s", type=float, default=None, help="Fix ArcFace scale s (otherwise tuned)")
    parser.add_argument("--arcface-m", type=float, default=None, help="Fix ArcFace margin m (otherwise tuned)")
    parser.add_argument("--num-classes", type=int, default=4, help="Number of classes for ArcFace layer")
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "See ./configs/optuna_labelclustering_arcface_hpo.json for a template. "
...
def _build_expected_distributions(
    *,
    ...
    if cli_args.arcface_s is None:
        expected["arcface_s"] = optuna_distributions.FloatDistribution(low=16.0, high=128.0, log=False)
    if cli_args.arcface_m is None:
        expected["arcface_m"] = optuna_distributions.FloatDistribution(low=0.1, high=0.7, log=False)
...
def objective(trial: optuna.Trial, cli_args: argparse.Namespace) -> float:
    ...
    try:
        ...
        if cli_args.arcface_s is not None:
            arcface_s = float(cli_args.arcface_s)
        elif resolved and "arcface_s" in resolved:
            arcface_s = float(resolved["arcface_s"])
        else:
            arcface_s = float(trial.suggest_float("arcface_s", 16.0, 128.0))

        if cli_args.arcface_m is not None:
            arcface_m = float(cli_args.arcface_m)
        elif resolved and "arcface_m" in resolved:
            arcface_m = float(resolved["arcface_m"])
        else:
            arcface_m = float(trial.suggest_float("arcface_m", 0.1, 0.7))
        ...
        base_hyperparameters = {
            ...
            "EPOCHS": epochs,
            "LEARNING_RATE": lr,
            "ARC_FACE_S": arcface_s,
            "ARC_FACE_M": arcface_m,
        ...
        for attempt_idx in range(max_attempts):
            ...
            dummy_args = SimpleNamespace(representation=representation)
            embedding_dim = get_embedding_dim(model, device, dummy_args)
            ...
            try:
                for epoch in range(epochs):
                    ...
                    if cli_args.amp and device.type == "cuda":
                        _train_loss, _train_acc = _train_one_epoch_arcface_amp(
                            model, metric_fc, train_loader, optimizer, device, criterion, epoch_label, scaler
                        )
                    else:
                        _train_loss, _train_acc = train_one_epoch_arcface(
                            model, metric_fc, train_loader, optimizer, device, criterion, epoch_label
                        )

                    if cli_args.amp and device.type == "cuda":
                        val_loss, val_acc = _evaluate_arcface_amp(model, metric_fc, val_loader, device, criterion)
                    else:
                        val_loss, val_acc = evaluate_arcface(model, metric_fc, val_loader, device, criterion)
                    ...
                    if math.isfinite(val_loss):
                        ...
                        if val_loss < best_val_loss:
                            ...
                            if cli_args.save_checkpoints:
                                ...
                                torch.save(
                                    metric_fc.state_dict(),
                                    Path(logger.output_dir) / f"best_metric_fc_{model_key}.pth",
                                )

                if cli_args.save_checkpoints:
                    torch.save(model.state_dict(), Path(logger.output_dir) / f"last_encoder_{model_key}.pth")
                    torch.save(metric_fc.state_dict(), Path(logger.output_dir) / f"last_metric_fc_{model_key}.pth")

                trial.set_user_attr("output_dir", logger.output_dir)
                trial.set_user_attr("best_epoch", best_epoch)
                trial.set_user_attr("best_val_acc", float(best_val_acc) if math.isfinite(best_val_acc) else None)
                ...
                try:
                    del model, metric_fc, optimizer, criterion, train_loader, val_loader, scaler
                except Exception:
                    pass
                gc.collect()
                if torch.cuda.is_available():
        ...
        try:
            del model, metric_fc, optimizer, criterion, train_loader, val_loader, logger
        except Exception:
            pass
        gc.collect()
        if torch.cuda.is_available():
...
def main() -> None:
    ...
    meta = {
        ...
        "arcface_s": cli_args.arcface_s,
        "arcface_m": cli_args.arcface_m,
        "num_classes": cli_args.num_classes,
