"""
小規模CNNオートエンコーダ（スペクトログラム入力）の学習・潜在空間可視化スクリプト。
learn_tool の共通ユーティリティと encoder.base_model の AutoEncoder_Small を利用して簡素化。
"""
...
# src ディレクトリをパスに追加（スクリプト単体実行時のモジュール解決用）
...
from encoder.base_model import AutoEncoder_Small


# ハイパーパラメータ
ENCODER_MODEL = 'CNN_small'
...
def reduce_latent_space(latent_matrix, classes, method, components, seed, logger):
    ...
    if method == 'LDA':
        reducer = LinearDiscriminantAnalysis(n_components=min(components, len(np.unique(classes)) - 1))
...
def main() -> None:
    ...
    model = AutoEncoder_Small().to(device)
...
VISUALIZATION = 't-SNE' # 可視化手法の選択 : t-SNE, PCA, LDA, UMAP, MDS
...
def prepare_dataloader(
        ...
        seed=SEED, data_csv_path=DATA_CSV_PATH, main_data_dir = f'./encoder/datasets/'
        ):
...
def main():
    ...
    if SAVE_LATENT_SPACE == 'y':
        ...
        if VISUALIZATION =='Projection-based PCA':
            # principal_axes の保存
            principal_axes_df = pd.DataFrame(principal_axes)
            principal_axes_file = os.path.join(output_dir, 'principal_axes.csv')
            principal_axes_df.to_csv(principal_axes_file, index=False)
            logger.info(f"principal_axes を {principal_axes_file} に保存しました。")
