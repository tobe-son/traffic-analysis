"""
VGG11ベースのCNNオートエンコーダ（スペクトログラム入力）を学習し潜在空間を可視化するスクリプト。
learn_tool ユーティリティと encoder.new_model の AutoEncoder_VGG11 を利用して構成を簡素化。
"""
...
from encoder.new_model import AutoEncoder_VGG11


# ハイパーパラメータ
ENCODER_MODEL = 'CNN_VGG11'
DATA_SELECTION = 'loc1'
...
VISUALIZATION = 'PCA'  # t-SNE, PCA, LDA, UMAP, MDS
...
def main() -> None:
    ...
    model = AutoEncoder_VGG11().to(device)
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        # 最終的に潜在チャネルへ圧縮
        self.final_conv = nn.Conv2d(256, 32, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x = self.initial(x)
        x = self.block1(x)
        # Store spatial resolution before pooling for decoder upsampling.
...
class Decoder(nn.Module):
    ...
    def forward(self, x, pool_shapes=None):
        if pool_shapes is None or len(pool_shapes) != 3:
            raise ValueError("Decoder requires the encoder's stored spatial shapes.")
